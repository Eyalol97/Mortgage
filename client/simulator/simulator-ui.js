...
(function () {
  ...
  function switchToMix(index) {
    ...
    // Restore form values — leave the solved field empty so it re-computes
  ...
  function triggerPdf() {
    const mixes = getMixes();
    if (mixes.length === 0) { showMessage('error', 'No mixes saved yet.'); return; }
  ...
  async function handleSaveMix() {
    ...
    try {
      ...
      showMessage('error', 'Could not validate. Please check your connection.');
    } finally {
  ...
  function init() {
    ...
    // Clicking a solved (readonly) field clears it so the user can type a new value
    FINANCIAL.forEach(id => {
      const el = document.getElementById(id);
      if (!el) return;
