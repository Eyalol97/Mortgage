...
// Inline formatters — shared/formatCurrency and formatDecimal are browser-only globals.
const _currency = v => '₪' + Math.round(v).toLocaleString('he-IL');
const _decimal  = v => Math.round(v * 10) / 10;
...
function buildSummaryBlock(doc, mix, mixLabel) {
  ...
  labelValue(doc, 'Annual Rate',      _decimal(mix.annualRate) + '%',                             left, doc.y);
  labelValue(doc, 'Property Price',   _currency(mix.propertyPrice),                              left, doc.y);
  labelValue(doc, 'Equity',           _currency(mix.equity),                                     left, doc.y);
  labelValue(doc, 'Loan Amount',      _currency(mix.loan),                                       left, doc.y);
  labelValue(doc, 'Duration',         mix.duration + ' years',                                   left, doc.y);

  doc.moveDown(0.5);
  sectionTitle(doc, mixLabel + ' — Key Results');
  labelValue(doc, 'Monthly Payment', _currency(mix.firstMonthlyPayment), left, doc.y);
  labelValue(doc, 'Total Interest',  _currency(mix.totalInterest),       left, doc.y);
  labelValue(doc, 'Total Payment',   _currency(mix.totalPayment),        left, doc.y);
}
...
function buildScheduleTable(doc, mix, mixLabel) {
  ...
  rows.forEach(row => {
    ...
    doc.text(_currency(row.payment),        COL.payment,   y, { width: W, continued: true });
    doc.text(_currency(row.principal),      COL.principal, y, { width: W, continued: true });
    doc.text(_currency(row.interest),       COL.interest,  y, { width: W, continued: true });
    doc.text(_currency(row.remainingBalance), COL.balance, y, { width: W });
  });

  if (truncated) {
    doc.moveDown(0.4)
       .fontSize(8).fillColor('#888888').font('Helvetica-Oblique')
...
function buildComparisonTable(doc, mixes) {
  ...
  const rows = [
    ...
    ['Annual Rate',      ...mixes.map(m => _decimal(m.annualRate) + '%')],
    ['Loan Amount',      ...mixes.map(m => _currency(m.loan))],
    ['Duration',         ...mixes.map(m => m.duration + ' years')],
    ['Monthly Payment',  ...mixes.map(m => _currency(m.firstMonthlyPayment))],
    ['Total Interest',   ...mixes.map(m => _currency(m.totalInterest))],
    ['Total Payment',    ...mixes.map(m => _currency(m.totalPayment))],
  ];
  ...
  rows.forEach(row => {
    const y = doc.y;
    row.forEach((cell, i) => {
      const isBest = i > 0 && i - 1 === bestIdx;
      if (isBest) doc.fillColor('#1a7a3c').font('Helvetica-Bold');
      doc.text(cell, left + i * colW, y, { width: colW, continued: i < row.length - 1 });
      if (isBest) doc.fillColor('#333333').font('Helvetica');
    });
  });

  doc.moveDown(0.4).fontSize(8).fillColor('#1a7a3c')
...
export async function generatePdf(req, res, next) {
  try {
    ...
    doc.fontSize(18).fillColor('#1a3c5e').font('Helvetica-Bold')
    ...
    mixes.forEach((mix, i) => {
      if (i > 0) doc.addPage();
      buildSummaryBlock(doc, mix, `Mix ${i + 1}`);
      doc.moveDown(0.5);
      buildScheduleTable(doc, mix, `Mix ${i + 1}`);
    });
